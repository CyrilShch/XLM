...
#   PARA_TGT_TEST=$PARA_PATH/Data_Limburgisch_Test_v5.txt
  ...
  PARA_SRC_VALID=$PARA_PATH/Data_Dutch_val_long_v4.txt
  PARA_TGT_VALID=$PARA_PATH/Data_Limburgish_val_long_v6.txt
  PARA_SRC_TEST=$PARA_PATH/Data_Dutch_Test_long_v4.txt
  PARA_TGT_TEST=$PARA_PATH/Data_Limburgisch_Test_long_v4.txt
fi
...
if [ "$SRC" == "en" -o "$TGT" == "en" ]; then
  ...
  cd $MONO_PATH/en
  wget -c https://www.dropbox.com/s/8im60bqeftbqjrd/corpus.limburgish_train1.gz
  wget -c https://www.dropbox.com/s/dswwk9vc3h9ckf8/corpus.limburgish_train2.gz
...
if ! [[ -f "$SRC_RAW" ]]; then
  ...
  cat $(ls $SRC/* | grep -v gz) | head -n $N_MONO > $SRC_RAW
fi
if ! [[ -f "$TGT_RAW" ]]; then
  echo "Concatenating $TGT monolingual data..."
  cat $(ls $TGT/* | grep -v gz) | head -n $N_MONO > $TGT_RAW
fi
...
wget -c https://www.dropbox.com/s/uftmpmrv3p53wel/dev.tar
